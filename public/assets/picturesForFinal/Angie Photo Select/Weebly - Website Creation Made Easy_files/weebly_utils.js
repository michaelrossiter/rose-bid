...
function showUpdateWhois(serviceID, paymentType, userServiceID, sld, tld, years, initialData, closeFunction, successFunction) {
	...
	for ( var name in initialData) {
		if (initialData.hasOwnProperty(name)) {
			var el = $("whois-" + name);
			if (el)
				el.value = initialData[name];
		}
	}
...
var pageDropDownOptions = '';
function createPageDropDown(id, options) {
	options = options || {};
	var pageSelect = $(id);
	pageDropDownOptions = '';
	_createPageDropDown(Weebly.PageManager.topLevelPages, "", "");
	pageSelect.insert(pageDropDownOptions);
	var first = pageSelect.down('option');
	if (options.current) {
		pageSelect.value = options.current;
	}
	else {
		pageSelect.value = first.value; //Will have the last item select otherwise
	}
	return new Chosen(pageSelect, options);
}


function _createPageDropDown(pages, spaces, dash) {
	pages.each(function(page) {
		pageDropDownOptions += '<option value="' + page.id + '" ' + (pageDropDownOptions.length == 0 ? 'selected' : '') + '>' + spaces + dash
			+ page.title + '</option>';
		if (page.children) {
			_createPageDropDown(page.children, (dash ? "&nbsp; &nbsp;" : '') + spaces, "- ");
		}
	});
}

function openUrlForwardingPrompt() {
	(new Weebly.Dialog('url-forwarding-prompt', {
		modal: true
	})).open();
	$('url-forwards').update('');
	new Ajax.Request(ajax, {
		parameters: {
			pos: 'loadurlpageforwards',
			cookie: document.cookie
		},
		onSuccess: function(t) {
			if (t.responseText && t.responseText.isJSON()) {
				var forwards = t.responseText.evalJSON();
				if (forwards.size() > 0) {
					forwards.each(function(forward) {
						newUrlForward(forward.old_url, forward.target_id);
					});
				}
				else {
					newUrlForward();
					$('url-forwards').down('input').select();
				}
			}
		}
	})
...
var urlForwardDropDowns = [];
var urlForwardDropDownOpened = false;
function newUrlForward(url, page_id) {
	var default_value = false;
	if (!url) {
		url = '/example.html';
		default_value = true;
	}
	var count = $$('.url-forward').size();
	var new_forward = '<div class="url-forward"><div class="url-forward-old"><input value="'
		+ url
		+ '" style="'
		+ (default_value ? 'color: #666;' : '')
		+ '" class="shaded-input" onclick="clearPrefilledInput(this, \'/example.html\');" onblur="formatUrlForward(this); return false;"></div><div class="url-forward-new"><select style="width:308px;" id="url-forward-select-'
		+ count + '"></select></div><a href="#" onclick="deleteUrlForward(this); return false;" class="delete-forward"></a></div>';
	$('url-forwards').insert({
		top: new_forward
	});
	var dd = createPageDropDown('url-forward-select-' + count, {
		current: page_id,
		onActivate: function(container) {
			urlForwardDropDownOpened = true;
		...
		onClose: function(container) {
			container.relativize().setStyle({
				position: ''
			});
			urlForwardDropDownOpened = false;
		}
	});
	urlForwardDropDowns.push(dd);
	Weebly.AllDialogs['url-forwarding-prompt'].positionDialog();
}

function closeAllForwardingDropDowns() {
	if (urlForwardDropDownOpened) {
		urlForwardDropDownOpened = false;
		urlForwardDropDowns.each(function(dd) {
			if (dd.active_field) {
				dd.close_field();
			}
		});
...
function formatUrlForward(el) {
	...
	if (url == '/index.html') {
		alert(_W.tl('You cannot redirect index.html'));
	}
}

function deleteUrlForward(el) {
	$(el).up('.url-forward').remove();
	Weebly.AllDialogs['url-forwarding-prompt'].positionDialog();
}

function saveUrlForwards() {
	var forwards = [];
	$$('.url-forward').each(function(el) {
		var old = el.down('input').value;
		if (old) {
			forwards.push({
				url: old,
				page_id: el.down('select').value
			});
		}
	})
	new Ajax.Request(ajax, {
		parameters: {
			pos: 'saveurlforwards',
			forwards: Object.toJSON(forwards),
			cookie: document.cookie
		}
	});
	Weebly.AllDialogs['url-forwarding-prompt'].close();
}
...
/**
 * Wraps the call to publishToFile on the Editor Settings accordion
 *
 * @param int siteId
 * @param int userId
 *
 * @return void
 */
function doPublishToFile( email )
{
	new Ajax.Request( ajax, {
		parameters: {
			pos: 'publishToFile',
		...
		onSuccess: function(t) {
			Weebly.SiteSettings.refreshPage('archive');
		}
	});
}

/**
 * Shows the publishToFile WeeblyDialog from the Editor Settings accordion
 *
 * @param string email
 *
 * @return void
 */
function showPublishToFile( email )
{
	if ( !email )
	{
		email = '';
	...
	dialog = new Weebly.Dialog( 'publishToFileModal', {
		modal: true,
		onClose: function(){
			$( 'publishToFileBeginButton' ).stopObserving( );
			$( 'publishToFileCancelButton' ).stopObserving( );
		}
	});

	$( 'publishToFileEmail' ).value = email;

	$( 'publishToFileBeginButton' ).observe("click", function(e) {
		doPublishToFile( $( 'publishToFileEmail' ).value );
		dialog.close( );
		e.preventDefault( );
	});

	$( 'publishToFileCancelButton' ).observe("click", function(e) {
		dialog.close( );
		e.preventDefault( );
	});
